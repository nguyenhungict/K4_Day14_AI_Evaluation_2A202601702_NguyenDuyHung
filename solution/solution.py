...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an expert evaluator. Score the AI answer for each rubric "
            "criterion on a scale from 0.0 to 1.0, then reply with ONLY a JSON "
            "object mapping each criterion name to its score.\n\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n\n"
            f"Rubric:\n{criteria_lines}\n\n"
            'Respond with JSON like {"criterion_name": 0.8, ...}.'
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Heuristic: the first-scored response in the batch consistently
        # comes out on top — a symptom of position bias rather than quality.
        positional_bias = (
            len(averages) > 1
            and averages[0] == max(averages)
            and averages[0] > averages[-1]
        )
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            ...
            fix = suggestions[index % len(suggestions)] if suggestions else "Review manually"
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            ...
            "irrelevant": "Improve prompt clarity and intent detection so answers directly address the question asked",
            "incomplete": "Increase chunk size or top_k in the RAG pipeline to reduce context fragmentation",
            "off_topic": "Add routing/guardrails to detect and correct off-topic responses before returning them",
            "refusal": "Relax overly strict guardrails and add few-shot examples for questions the agent should answer",
        }

        suggestions = [
            suggestion_map[category] for category in categories if category in suggestion_map
        ]

        fallback_suggestions = [
            "Add few-shot examples showing complete, grounded answers to guide generation",
            "Expand the golden dataset with these failure cases to prevent regressions",
            "Review retrieval ranking (Context Precision) to ensure the most relevant chunk is used first",
        ]
